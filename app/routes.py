...
import secrets
from . import db
from .models import User, Design, Poll, PollOption, Vote, Comment, DesignCheckRequest, QRCode
from .forms import RegistrationForm, LoginForm, EditProfileForm, DesignUploadForm, CreatePollForm, CommentForm, VoteForm, AddDesignsToPollForm
...
def forum():
    ...
    # Her anket için seçenek sayısını hesapla ve tarih bilgisi ekle
    today = date.today()
    for poll in polls:
        poll.option_count = poll.options.count()
        poll.is_today = poll.created_at.date() == today
    return render_template('forum.html', polls=polls, today=today)
